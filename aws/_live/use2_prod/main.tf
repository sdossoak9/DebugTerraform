...
module "ApiGateway" {
  ...
  aws_api_gateway_rest_api = "oak9-${var.stack}-lambdaAndConsoleApi-gateway"
...
module "AzureApiGateway" {
  ...
  base_path                = var.base_path
  stage_name               = "prod"
  aws_api_gateway_rest_api = "oak9-AzureMarketplace"
  lambda_details           = module.Lambda.functions_details
  acm_certificate_arn      = data.aws_acm_certificate.oak9CloudVirginia.arn
}
...
module "Websocket" {
  ...
  stage_name                    = var.stage_name
  base_path                     = var.base_path
  aws_api_gateway_websocket_api = "oak9-${var.stack}-WebsocketOrchestrator"
  lambda_details                = module.Lambda.functions_details
...
module "CloudFrontAzure" {
  ...
  environment         = var.environment
  stack               = var.stack
  ui_s3Bucket         = "marketplace.${var.dns_name}.${var.region}-azure-ui-assets"
  root_object_path    = "/ui-deployment"
  acm_certificate_arn = data.aws_acm_certificate.oak9CloudVirginia.arn
}
...
#this differs in var.env for other accounts becuase there is a dependency on chaning the bootstrap
#backup s3 buckets CLOUD-3035, for now this fix will work till we create another ticket to fix backup buckets to use var.environment
module "Lambda" {
...
module "S3ServerAccess" {
  source        = "../../_modules/s3"
  bucket_name   = "oak9-s3-server-access"
  force_destroy = true
}